...
第一階段是\textbf{投資組合配置決策}：在每一個再平衡日 $t$，投資組合經理人需從候選股池中選出恰好 $K$ 檔股票，並決定各股票之目標權重 $w_i$。配置結果須滿足權重總和、持股檔數、單股上下限、產業／主題集中度、Beta、流動性、周轉率與 CVaR 風險限制；目標則是最大化 $\mu^\top w$，其中 $\mu_i$ 表示股票 $i$ 的事前吸引力分數。

第二階段是\textbf{交易落地決策}：已知第一階段給出之理想權重 $w^\star$ 與持股名單 $z^\star$，並給定期初張數 $x^0$、期初現金 $C^0$ 與各檔目前每張價格 $P_i$，決策者要決定每檔股票應買入張數 $u_i$、賣出張數 $v_i$、最終持有張數 $x_i$ 與剩餘現金 $C$。此階段須滿足庫存守恆、現金非負、持股名單延用 Stage 1、買賣互斥，且所有交易張數皆為非負整數。目標是最小化實際相對權重與理想 $w^\star$ 的 L1 偏離，並對交易標的數加入小幅懲罰以避免零碎下單。

\textbf{原本人工 vs.\ 系統化。}{}\Cref{tab:auto} 摘要原流程中需人工判斷的環節，以及本研究如何以最佳化模型將其系統化。
...
令 $i \in I = \{1, \dots, N\}$ 為候選股，$j \in J = \{1, \dots, M\}$ 為產業／主題群組，$S_j \subseteq I$ 為群組 $j$ 內股票集合，$s \in \Omega$ 為 CVaR 歷史情境。主要參數包含投資吸引力分數 $\mu_i$、Beta $\beta_i$、期初權重 $w_i^0$、單股下／上限 $L, U$、持股檔數 $K$、群組上限 $\gamma_j$、流動性上限 $\ell_i$、周轉率上限 $\tau$、情境報酬 $r_{i,s}$、信賴水準 $\alpha$ 與 CVaR 上限 $\bar{C}^{\text{risk}}$。決策變數為目標權重 $w_i$、選股變數 $z_i \in \{0,1\}$、周轉率線性化變數 $\delta_i^+, \delta_i^-$、VaR 輔助變數 $\eta$ 與情境超額損失 $\xi_s$。

\textbf{Stage 1 模型：}
\begin{align}
\max_{w, z, \delta^\pm, \eta, \xi}\quad & \sum_{i \in I} \mu_i w_i \label{obj1}\\
\text{s.t.}\quad
& \sum_{i \in I} w_i = 1, & & \sum_{i \in I} z_i = K, \label{c1:budget}\\
& L z_i \leq w_i \leq U z_i,\ \forall i \in I, & & w_i \leq \ell_i,\ \forall i \in I, \label{c1:box}\\
& \sum_{i \in S_j} w_i \leq \gamma_j,\ \forall j \in J, & & \sum_{i \in I} \beta_i w_i \leq \beta^{\max}, \label{c1:ind}\\
& w_i - w_i^0 = \delta_i^+ - \delta_i^-,\ \forall i \in I, & & \sum_{i \in I} (\delta_i^+ + \delta_i^-) \leq \tau, \label{c1:turn}\\
& \xi_s \geq -\sum_{i \in I} r_{i,s} w_i - \eta,\ \forall s \in \Omega, & & \eta + \frac{1}{(1-\alpha)|\Omega|}\sum_{s \in \Omega} \xi_s \leq \bar{C}^{\text{risk}}, \label{c1:cvar}\\
& w_i, \delta_i^\pm, \xi_s \geq 0,\ \eta \in \mathbb{R}, & & z_i \in \{0,1\},\ \forall i \in I. \nonumber
\end{align}

其中 \Cref{c1:turn} 將 $|w_i - w_i^0|$ 拆成正負兩個非負變數，以線性化周轉率限制；\Cref{c1:cvar} 為 Rockafellar–Uryasev CVaR 線性化形式。

\subsection{Stage 2：交易落地與整數張數轉換}

給定 Stage 1 的解 $(w^\star, z^\star)$，Stage 2 將理想權重轉換為可執行交易張數。令 $P_i$ 為股票 $i$ 之每張價格（元；1 張 $= 1{,}000$ 股）、$x_i^0$ 為期初持有張數、$C^0$ 為期初現金、$c_b, c_s$ 為買入／賣出交易成本率（後者含證交稅），且 $V^0 = C^0 + \sum_i P_i x_i^0$ 為期初總資產價值。若 $z_i^\star = 1$，股票 $i$ 之最低與最高持有張數分別為 $L_i^{\text{lot}} = \lceil L V^0 / P_i \rceil$ 與 $U_i^{\text{lot}} = \lfloor U V^0 / P_i \rfloor$；另定義 $\bar{u}_i = \max(0,\, U_i^{\text{lot}} - x_i^0)$ 為本期最大可買入張數，並以 $\lambda_{\text{trade}} \geq 0$ 為交易標的數之懲罰係數。決策變數為調倉後最終持有張數 $x_i$、買入張數 $u_i$、賣出張數 $v_i$、剩餘現金 $C$、買賣指示變數 $B_i, Q_i \in \{0,1\}$ 與權重絕對偏離 $d_i$。
...
& \mathrlap{C = C^0 + \sum_{i \in I} P_i v_i (1-c_s) - \sum_{i \in I} P_i u_i (1+c_b), \quad C \geq 0} \label{c2:cash}\\
& d_i \geq P_i x_i / V^0 - w_i^\star,\ \forall i \in I, & & d_i \geq w_i^\star - P_i x_i / V^0,\ \forall i \in I \label{c2:dev}\\
& \mathrlap{L_i^{\text{lot}} z_i^\star \leq x_i \leq U_i^{\text{lot}} z_i^\star, \quad \forall i \in I} \label{c2:lot}\\
& u_i \leq \bar{u}_i B_i,\ \ v_i \leq x_i^0 Q_i,\ \forall i \in I, & & B_i + Q_i \leq 1,\ \forall i \in I \label{c2:mutex}\\
& \mathrlap{x_i, u_i, v_i \in \mathbb{Z}_{\geq 0}; \; B_i, Q_i \in \{0,1\};\; d_i, C \geq 0.} \nonumber
\end{align}
\Cref{c2:inv,c2:cash} 分別表示庫存與現金守恆；\Cref{c2:dev} 線性化實際權重與理想權重的 L1 偏離；\Cref{c2:mutex} 限制同一股票不能同時買入與賣出。為維持兩階段一致性，$L_i^{\text{lot}}, U_i^{\text{lot}}$ 亦可反向投影回 Stage 1 的權界 $L_i = L_i^{\text{lot}} P_i / V^0$、$U_i = U_i^{\text{lot}} P_i / V^0$（見 \cref{sec-data}）。

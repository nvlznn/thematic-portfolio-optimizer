...
\text{s.t.}\quad
& \sum_{i\in I}w_i=1, 
&& \sum_{i\in I}z_i=K, \label{c1:basic}\\[-0.5mm]
& Lz_i\le w_i\le Uz_i,\quad \forall i\in I,
&& w_i\le \ell_i,\quad \forall i\in I, \label{c1:boxliq}\\[-0.5mm]
& \sum_{i\in S_j}w_i\le \gamma_j,\quad \forall j\in J,
&& \sum_{i\in I}\beta_iw_i\le \beta^{\max}, \label{c1:indbeta}\\[-0.5mm]
& w_i-w_i^0=\delta_i^+-\delta_i^-,\quad \forall i\in I,
&& \sum_{i\in I}(\delta_i^++\delta_i^-)\le \tau, \label{c1:turn}\\[-0.5mm]
& \xi_s\ge -\sum_{i\in I}r_{i,s}w_i-\eta,\quad \forall s\in\Omega,
&& \eta+\frac{1}{(1-\alpha)|\Omega|}\sum_{s\in\Omega}\xi_s\le \bar C^{\text{risk}}, \label{c1:cvar}\\[-0.5mm]
...
給定 Stage 1 的解 $(w^\star,z^\star)$，Stage 2 將理想權重轉換為可執行交易張數。令 $P_i$ 為每張價格、$x_i^0$ 為期初張數、$C^0$ 為期初現金、$c_b,c_s$ 為買入／賣出交易成本率，且 $V^0=C^0+\sum_iP_ix_i^0$ 為期初總資產。若股票被選入，最低與最高持有張數分別為 $L_i^{\text{lot}}=\lceil LV^0/P_i\rceil$ 與 $U_i^{\text{lot}}=\lfloor UV^0/P_i\rfloor$；另定義 $\bar u_i=\max(0,U_i^{\text{lot}}-x_i^0)$ 為最大可買入張數。
...
\text{s.t.}\quad
& x_i=x_i^0+u_i-v_i,\quad \forall i\in I, \label{c2:inv}\\[-0.5mm]
& C=C^0+\sum_{i\in I}P_iv_i(1-c_s)-\sum_{i\in I}P_iu_i(1+c_b), 
&& C\ge0, \label{c2:cash}\\[-0.5mm]
& d_i\ge \frac{P_ix_i}{V^0}-w_i^\star,\quad \forall i\in I,
&& d_i\ge w_i^\star-\frac{P_ix_i}{V^0},\quad \forall i\in I, \label{c2:dev}\\[-0.5mm]
& L_i^{\text{lot}}z_i^\star\le x_i\le U_i^{\text{lot}}z_i^\star,\quad \forall i\in I, \label{c2:lot}\\[-0.5mm]
& u_i\le \bar u_iB_i,\quad v_i\le x_i^0Q_i,\quad \forall i\in I,
&& B_i+Q_i\le1,\quad \forall i\in I, \label{c2:bq}\\[-0.5mm]
...
\begin{figure}[!hbt]
\centering
\includegraphics[width=0.98\linewidth]{top-k-flowchart.png}
...
\begin{figure}[!hbt]
\centering
\includegraphics[width=0.98\linewidth]{greedy-flowchart.png}
\caption{Greedy 多起點流程：起點 A 與起點 B 各經 \textsc{Allocate}+\textsc{Repair} 與 1-swap \textsc{LocalSearch}，最後取目標值較高者。}
\label{fig:greedy}
\end{figure}
